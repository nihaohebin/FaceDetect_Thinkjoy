...
import com.tjoydoor.view.camera.FaceOverlayDetect;
 ...
 * 横屏手机 实时检测识别
 */
public class MainPhone_Detect_LActivity extends Activity implements SurfaceHolder.Callback, Camera.PreviewCallback {


    @BindView(R.id.surfaceView)
    SurfaceView surfaceView;
    @BindView(R.id.overlay)
    FaceOverlayDetect overlay;
            ...
            switch (msg.arg1) {
                case 1:
                    ...
                    if (isPosting) {
                        ...
                        final long postStartTime = System.currentTimeMillis();
                        timeS.add(postStartTime);

                        //人脸识别
                        SearchManage.newInstance(MainPhone_Detect_LActivity.this).faceSearch(faceCroped, Global.Varibale.faceSetId, 1, new FaceSearchListener() {
                            @Override
...
//                                Logger.i("HTTP请求时间:" + httpTime + "ms  ");
    ...
    public void surfaceCreated(SurfaceHolder surfaceHolder) {
        try {
            ...
            if (mCamera == null) {
                mCamera = Camera.open(SPUtil.getInstance(MainPhone_Detect_LActivity.this).getInt(Global.Const.cameraId));
            }

            Camera.getCameraInfo(SPUtil.getInstance(MainPhone_Detect_LActivity.this).getInt(Global.Const.cameraId), cameraInfo);
            ...
            e.printStackTrace();
            ToastUtil.showMessage(MainPhone_Detect_LActivity.this, " Fail to connect to camera service");
        }
    }
    ...
    public void onPreviewFrame(final byte[] data, final Camera camera) {
        if (!isThreadWorking) {
            ...
            ThreadPoolHelp.Builder.cached().builder().execute(new Runnable() {
                        @Override
                        public void run() {
                            Logger.e("线程是 = " + Thread.currentThread().getId());
                            ...
                            Camera.getCameraInfo(SPUtil.getInstance(MainPhone_Detect_LActivity.this).getInt(Global.Const.cameraId), info);
...
//            });
    ...
    private void configureCamera(int width, int height) {
        ...
        overlay.setSupportedSceneModes(parameters.getSupportedSceneModes() + "");
        overlay.setExposureCompensation(parameters.getExposureCompensation() + "", parameters.getMaxExposureCompensation() + "");
        overlay.setZoom(parameters.getZoom() + "", parameters.getMaxZoom() + "");
    ...
    private void setDisplayOrientation() {
        ...
        mDisplayRotation = ScreenUtil.getDisplayRotation(MainPhone_Detect_LActivity.this);
        mDisplayOrientation = ScreenUtil.getDisplayOrientation(mDisplayRotation, SPUtil.getInstance(MainPhone_Detect_LActivity.this).getInt(Global.Const.cameraId));
