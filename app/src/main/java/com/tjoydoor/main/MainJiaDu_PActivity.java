...
public class MainJiaDu_PActivity extends Activity implements OnBannerListener, SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mainjiadu_p);
    ...
    public void surfaceCreated(SurfaceHolder surfaceHolder) {
        try {
            ...
            if (mCamera == null) {
                mCamera = Camera.open(SPUtil.getInstance(MainJiaDu_PActivity.this).getInt(Global.Const.cameraId));
            }

            Camera.getCameraInfo(SPUtil.getInstance(MainJiaDu_PActivity.this).getInt(Global.Const.cameraId), cameraInfo);
            ...
            e.printStackTrace();
            ToastUtil.showMessage(MainJiaDu_PActivity.this, "相机启动失败");
        }
    }
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        ...
        try {
            ...
            e.printStackTrace();
            ToastUtil.showMessage(MainJiaDu_PActivity.this, " Fail to connect to camera service");
        }
    }
        ...
        if (!isThreadWorking) {
            ...
            ServiceThreadPoolManager.getInstance().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    Camera.getCameraInfo(SPUtil.getInstance(MainJiaDu_PActivity.this).getInt(Global.Const.cameraId), info);
...
//                                    ImageUtil.saveBitmap2file(faceCroped, TimeUtil.getCurrentTime()+".jpg");
                    ...
                    if (Global.Varibale.isFirst) {
                        ...
                        FileUtil.saveDataToFile(MainJiaDu_PActivity.this, "w = " + bitmap.getWidth() + "  h = " + bitmap.getHeight() + "<br>", Global.FilePath.bmpSize);
                    }

                    //只记录十次android处理时间
                    if (++Global.Varibale.saveAndroidDealTimeCount <= 10) {
                        if (Global.Varibale.isSaveAndroidDealTime) {
                            FileUtil.saveDataToFile(MainJiaDu_PActivity.this, (System.currentTimeMillis() - myStartTime) + "ms ", Global.FilePath.androidDealTime);
                        }
                    } else {
            ...
            switch (msg.arg1) {
                case 1:
                    ...
                    if (isPosting) {
                        ...
                        SearchManage.newInstance(MainJiaDu_PActivity.this).faceSearch(faceCroped, Global.Varibale.faceSetId, 1, new FaceSearchListener() {
                            @Override
                            public void onFaceSearchListener(FaceSearchInfo search, ErrorMsg error) {

                                final String httpTime = String.valueOf((System.currentTimeMillis() - startTime));
//                                Logger.i("HTTP请求时间:" + httpTime + "ms  ");
                                ...
                                if (++Global.Varibale.saveHttpRequestTimeCount <= 10) {
                                    if (Global.Varibale.isSaveHttpRequestTime) {
                                        FileUtil.saveDataToFile(MainJiaDu_PActivity.this, (httpTime + "ms  "), Global.FilePath.httpTime);
                                    }
                                } else {
                                ...
                                if (error.getCode() == 0) {
                                    ...
                                    final double confidence = search.getResultFace().get(0).getConfidence();
                                    final String name = search.getResultFace().get(0).getPersonId();
                                    ...
                                    runOnUiThread(new Runnable() {
                                        ...
                                        public void run() {
                                            if (confidence >= 0.9) {
                                                tvName.setText(name);
                                                tvConfidence.setText("相似度：" + confidence + "");
                                                tvNet.setText("网络延时：" + httpTime + "ms");
                                            } else {
                                                tvName.setText("正在识别...");
                                                tvConfidence.setText("正在识别...");
                                                tvNet.setText("正在识别...");
                                            }
                                        }
                                    });
                                    ...
                                    FileUtil.saveDataToFile(MainJiaDu_PActivity.this, "名 字:" + name + "-----相似度：" + confidence + "---msgId：" + error.getMsg_id() + "<br>", Global.FilePath.compareResult);
            ...
            switch (msg.arg1) {
                case 1:
                    ...
                    switch (msgType) {
                        ...
                        case 0:
                            List<UserInfo> userInfos = SPUtil.getInstance(MainJiaDu_PActivity.this).getUserInfoList();
                            ...
                            if (null != userInfos && userInfos.size() != 0) {
                                for (UserInfo userInfo : userInfos) {
                                    ...
                                    if (userInfo.getUserName().equals(successName)) {
                                        ...
                                        if (!TextUtil.isEmpty(userInfo.getPicPath())) {
                                            GlideUtil.loadCircleImage(MainJiaDu_PActivity.this, userInfo.getPicPath(), ivHead);
                                        }
                                    }
                                }
                                ...
                                ToastUtil.showMessage(MainJiaDu_PActivity.this, "请补全该职员信息");
                            }
                ...
                case 2:
                    try {
                        if (faces != null) {
                            ...
                            tvFPS.setText("FPS：" + df2.format(fps));
...
//            Logger.i( previewSizes.get(i).width + "*" + previewSizes.get(i).height);
        ...
        tvScreen.setText("分辨率：" + previewWidth + "*" + previewHeight);
    }
    ...
    private void setDisplayOrientation() {
        ...
        mDisplayRotation = ScreenUtil.getDisplayRotation(MainJiaDu_PActivity.this);
    ...
    private void doSexDetect() {
        ...
        DetectManage.newInstance(MainJiaDu_PActivity.this).getFaceRegion(faceCroped, true, true, new FaceRegionCheckListener() {
            @Override
