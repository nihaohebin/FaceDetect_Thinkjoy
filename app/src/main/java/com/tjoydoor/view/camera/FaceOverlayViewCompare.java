...
public class FaceOverlayViewCompare extends View {
    ...
    private void initialize() {
        ...
        if (Global.Varibale.isBigText) {
            stroke = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 4, metrics);
            size = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40, metrics);
        } else {
            stroke = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, metrics);
            size = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 20, metrics);
        }
        mPaint = new Paint();
